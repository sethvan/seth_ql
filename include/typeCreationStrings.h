...
struct TurnerMap {
    ...
    [[nodiscard]] constexpr Value at( const Key& key ) const {
        const auto it = std::find_if( data.begin(), data.end(), [&key]( const auto& v ) { return v.first == key; } );
        if ( it != data.end() ) {
            return it->second;
        }
        else {
...
constexpr auto typeCreationStrings = TurnerMap<std::string_view, DataType, userInputTypes.size()>{ { userInputTypes } };
